...
func TestConnections(t *testing.T) {
	for i, namespace := range []string{"", "custom_namspce"} {
		namespace := namespace
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {

			opts := []Option{
				WithConnectionNamespace(namespace),
			}

			redisOptions, closer := testRedis(t)
			defer closer()

			flushConn, err := OpenConnectionWithRedisOptions("conns-flush", redisOptions, nil, opts...)
			...
			connection, err := OpenConnectionWithRedisOptions("conns-conn", redisOptions, nil, opts...)
			assert.NoError(t, err)
			...
			conn1, err := OpenConnectionWithRedisOptions("conns-conn1", redisOptions, nil, opts...)
			assert.NoError(t, err)
			connections, err = connection.getConnections()
			assert.NoError(t, err)
...
func TestConnectionQueues(t *testing.T) {
	...
	for i, namespace := range []string{"", "custom_namspce"} {
		...
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			connection, err := OpenConnectionWithRedisOptions("conn-q-conn", redisOptions, nil, opts...)
			assert.NoError(t, err)
			require.NotNil(t, connection)
